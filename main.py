...
async def prod_ord(id: Optional[int]=None):
    ...
    if data == None or not data:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    return JSONResponse(content = {"orders":[{"id": f"{x['OrderId']}".strip(), "customer": f"{x['CompanyName']}","quantity":f"{x['Quantity']}","total_price":round((x['UnitPrice']*x['Quantity']) - (x['Discount']*(x['UnitPrice']*x['Quantity'])),2)}for x in data]}, status_code=status.HTTP_200_OK)
